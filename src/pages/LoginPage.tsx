...
export const LoginPage = () => {
  ...
  const { data: role, isLoading: isRoleLoading } = useRoleUser(userId);
  ...
  useEffect(() => {
    if (session && role) {
      ...
      if (role === 'admin') {
        navigate('/dashboard', { replace: true }); // ✅ admin va a dashboard
      } else {
        navigate('/', { replace: true }); // usuario normal va a /
      }
    }
  }, [session, role, navigate]);

  if (isSessionLoading || isRoleLoading) return <Loader />;
